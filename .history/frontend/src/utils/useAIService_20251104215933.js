...
// Giả định: useAxios.js nằm cùng thư mục utils/
import { useAxios } from './useAxios'; 

/**
 * Custom Hook chuyên biệt để gọi API Tóm tắt AI.
 * Giúp cô lập logic AI, caching, và xử lý lỗi vào một nơi duy nhất.
 */
export const useAIService = () => {
    // Sử dụng hook useAxios của bạn để thực hiện lời gọi HTTP
    const { apiCall } = useAxios(); // Giả định useAxios cung cấp hàm apiCall để gọi API
    const [summary, setSummary] = useState(null);
    const [isLoading, setIsLoading] = useState(false);
    const [error, setError] = useState(null);

    /**
     * Hàm gọi API Backend để lấy bản tóm tắt AI.
