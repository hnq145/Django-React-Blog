 ...
 * Xử lý logic nghiệp vụ cho tính năng Tóm tắt Bài viết bằng AI.
 * Đảm bảo logic caching và gọi API Backend (Django).
 */
export const useAIService = () => {
    // Gọi Custom Hook Axios cơ bản của bạn (Giả định nó xử lý JWT và Base URL)
    const api = useAxios(); 
    
    // State để quản lý kết quả, trạng thái tải, và lỗi
    const [summary, setSummary] = useState(null);
    const [isLoading, setIsLoading] = useState(false);
    const [error, setError] = useState(null);

    /**
     * Hàm chính để gọi API Backend Django yêu cầu tóm tắt.
    ...
    const getSummary = async (postId) => {
        ...
        // API Endpoint đã thống nhất: POST /posts/{post_id}/summarize/
