```{react:AI Chat Assistant (Tái sử dụng):frontend/src/views/partials/AIChatAssistant.jsx
import React, { useState, useRef, useEffect } from 'react';
// Import Hook AI Service chúng ta vừa tạo
import { useAIService } from '../../utils/useAIService'; 

/**
 * Component AIChatAssistant
 * * Mục đích: 
 * 1. Cung cấp giao diện Chatbox linh hoạt (FR-AI-01).
 * 2. Nhận bối cảnh bài viết (contextString) từ component cha (FR-AI-02).
 * 3. Cho phép người dùng nhập yêu cầu tùy chỉnh (Prompt).
 * 4. Gọi 'useAIService' để xử lý yêu cầu (Text/Image) (FR-AI-03, FR-AI-04).
 * 5. Hiển thị kết quả hoặc lỗi (NFR-02).
 */
const AIChatAssistant = ({ contextString }) => {
    // Sử dụng Hook AI Service
    const { isLoading, error: apiError, generateContent } = useAIService();

    // State nội bộ của Chatbox
    const [prompt, setPrompt] = useState(''); // Ô input của người dùng
    const [messages, setMessages] = useState([]); // Lịch sử chat (chỉ ở Frontend)
    ...
    // Xử lý khi người dùng nhấn Gửi (Submit)
