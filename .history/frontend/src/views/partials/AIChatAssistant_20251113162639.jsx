    ...
    return (
        <div className="flex flex-col h-screen border rounded-lg shadow-lg bg-white" style={{ maxHeight: '500px' }}>
            ...
            <div className="p-2 border-b bg-gray-50 rounded-t-lg">
                <h3 className="font-semibold text-sm-lg text-gray-800">Contextual AI Assistant 🤖</h3>
            </div>
            ...
            {/* Footer Input */}
            <form onSubmit={handleSubmit} className="border-t bg-gray-50 rounded-b-lg">
                <div className="flex space-x-2">
                    <input
                        type="text"
                        value={prompt}
                        onChange={(e) => setPrompt(e.target.value)}
                        placeholder="AI requests"
                        className="flex-grow p-1 border border-gray-300 rounded-lg text-center"
                        disabled={isLoading}
                    />
                    <button
                        type="submit"
                        className={`p-2 rounded-lg text-white transition duration-300 ${isLoading ? 'bg-gray-500' : 'bg-blue-600 hover:bg-blue-700'}`}
                        disabled={isLoading}
                    >
                        {/* Send Icon */}
