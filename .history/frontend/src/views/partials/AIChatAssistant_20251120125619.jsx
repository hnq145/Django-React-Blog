...
const AIChatAssistant = ({ contextString }) => {
    ...
    return (
        <div className="flex flex-col h-full border rounded-lg shadow-lg bg-white w-full max-w-2xl mx-auto min-h-[400px] max-h-[80vh]">
         ...
         <form onSubmit={handleSubmit} className="p-2 border-t bg-gray-50 rounded-b-lg">
                {/* Thay flex bằng grid:
                    - grid-cols-[1fr_auto]: Cột 1 (input) chiếm 1 phần không gian còn lại (1fr), Cột 2 (button) tự động theo kích thước nút (auto).
                    - w-full: Bắt buộc container chiếm 100% chiều rộng form.
                 */}
                <div className="grid grid-cols-[1fr_auto] gap-2 w-full items-center">
                    <input
                        ...
                        // w-full để nó lấp đầy cột 1fr của grid
                        // min-w-0 rất quan trọng để ngăn nó bị tràn
                        className="w-full min-w-0 p-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                        disabled={isLoading}
                    />
                    <button
                        type="submit"
                        // Không cần shrink-0 nữa vì grid auto đã lo việc đó
                        className={`p-2 rounded-lg text-white transition-colors duration-300 flex items-center justify-center ${isLoading ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700'}`}
                        disabled={isLoading}
                    >
