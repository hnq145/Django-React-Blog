...
const AIChatAssistant = ({ contextString }) => {
    ...
    return (
        // THAY ĐỔI 1: Thêm 'overflow-hidden' vào khung chính để chắc chắn không gì thò ra được
        <div className="flex flex-col h-full border rounded-lg shadow-lg bg-white w-full max-w-2xl mx-auto min-h-[400px] max-h-[80vh] overflow-hidden box-border">
            ...
            {/* Footer Input - PHẦN SỬA QUAN TRỌNG */}
            {/* 1. Biến form thành flex container luôn */}
            {/* 2. Thêm box-border để padding không làm phình width */}
            <form 
                onSubmit={handleSubmit} 
                className="flex items-center gap-2 p-2 border-t bg-gray-50 rounded-b-lg w-full box-border shrink-0"
            >
                <input
                    type="text"
                    value={prompt}
                    ...
                    // width: 100% (w-full) nhưng bị giới hạn bởi flex cha
                    // min-w-0: cốt lõi để không bị tràn
