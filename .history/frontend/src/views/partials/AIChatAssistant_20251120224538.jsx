        ...
        <div className="flex flex-col h-full border rounded-lg shadow-lg bg-white w-full max-w-2xl mx-auto min-h-[400px] max-h-[80vh] overflow-hidden box-border">
            ...
            <div className="flex-grow p-4 overflow-y-auto space-y-4 bg-gray-100 scroll-smooth">
                {messages.map((msg, index) => (
                    <div
                        key={index}
                        // Thêm w-full vào đây để container bao ngoài luôn chiếm hết chiều ngang
                        className={`flex flex-col w-full ${msg.role === 'user' ? 'items-end' : 'items-start'}`}
                    >
                        <div
                            // SỬA Ở ĐÂY:
                            // 1. w-fit: Để nó ưu tiên mở rộng chiều ngang
                            // 2. min-w-[100px] hoặc min-w-[20%]: Đảm bảo không bao giờ bị bóp quá bé
                            // 3. Đổi whitespace-pre-wrap thành whitespace-pre-line
                            className={`p-3 rounded-xl max-w-[85%] w-fit min-w-[100px] shadow break-words ${getMessageStyle(msg.role)}`}
                        >
                            {msg.type === 'image' ? (
                                <img
                                    src={msg.content}
                                ...
                                // Dùng pre-line thay vì pre-wrap để tránh bị lỗi cột dọc
