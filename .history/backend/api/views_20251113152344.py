...
class SummarizePostView(APIView):
    ...
    def post(self, request, post_id):
        ...
        try:
            cached_summary = AI_Summary.objects.get(post=post)
            
            return Response({
                "summary": cached_summary.summarized_content,
                "source": "cache"
        ...
        try:
            ...
            summary_text = ai_client.get_summary(post.content)

            if "Error" in summary_text or "unavailable" in summary_text:
               
                return Response(
                    {"detail": summary_text, "source": "ai_error"}, 
            ...
            return Response(
                {"detail": "Unknown system error while processing AI."},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
