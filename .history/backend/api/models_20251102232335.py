...
class AI_Summary(models.Model):
    """
    Model lưu trữ bản tóm tắt AI đã tạo cho một bài viết (Cơ chế Caching).
    Mối quan hệ 1-1 với Post (theo ERD).
    """
    # Mối quan hệ 1-1 với Post
    post = models.OneToOneField(
        'Post',  # Thay thế 'Post' bằng tên Model Post chính xác của bạn nếu khác
        on_delete=models.CASCADE,
        related_name='ai_summary',
        verbose_name="Bài viết được tóm tắt"
    )
    
    # Nội dung tóm tắt (Text field theo ERD)
    summarized_content = models.TextField(
        verbose_name="Nội dung tóm tắt AI"
    )
    
    # Thời gian tạo bản cache (để kiểm tra độ cũ nếu cần)
